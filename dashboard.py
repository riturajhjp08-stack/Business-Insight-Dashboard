...
def render_filtered_table(data):
    ...
    if 'Order Date' in display_df.columns:
        display_df['Order Date'] = pd.to_datetime(display_df['Order Date'], errors='coerce').dt.date
    if 'Ship Date' in display_df.columns:
        display_df['Ship Date'] = pd.to_datetime(display_df['Ship Date'], errors='coerce').dt.date
...
column_mapping = {}
if missing:
    # Silently fall back to generic explorer mode for non-matching datasets.
    generic_mode = True
